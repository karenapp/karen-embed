...
Karen._util.domReady = function(callback) {
    var ready = false;
...
Karen._util.assign = function assign(target, varArgs) { // .length of function is 2
  'use strict';
...
(function() {
  ...
  Karen._url.extractQueryStringParams = function(url) {
  ...
  Karen._url.stripQuery = function(url) {
    return url.split('?')[0];
  };

}).call(this);
(function() {
  Karen._util.snakeCaseKeys = function(options) {
  ...
  Karen._util.pick = function(options, keyWhitelist) {
...
(function() {
  ...
  Karen._autoLoadInlineWidgets = function() {
    return Karen._util.domReady(function() {
      return createInlineWidgets();
    });
  };

  Karen.initBadgeWidget = function(options) {
    return Karen._util.domReady(function() {
      return createBadgeWidget(options);
    });
  };

  Karen.destroyBadgeWidget = function() {
    if (!Karen.badgeWidget) {
      return;
    }
    Karen.badgeWidget.destroy();
    return delete Karen.badgeWidget;
  };

  Karen.initPopupWidget = function(options) {
    return Karen._util.domReady(function() {
      return Karen.showPopupWidget(options.url, 'PopupText', options);
    });
  };

  Karen.initInlineWidget = function(options) {
    if (!options.url) {
      return;
    }
    ...
    return Karen._util.domReady(function() {
      ...
      return new Karen.Iframe(options);
    });
  };

  Karen.showPopupWidget = function(url, embedType, options) {
    var onCloseHandler;
    if (embedType == null) {
      embedType = 'PopupText';
    }
    ...
    onCloseHandler = function() {
      return delete Karen.popupWidget;
    };
    Karen.popupWidget = new Karen.PopupWidget(url, onCloseHandler, embedType, options);
    return Karen.popupWidget.show();
  };

  Karen.closePopupWidget = function() {
    if (!Karen.popupWidget) {
      return;
    }
    return Karen.popupWidget.close();
  };
  ...
  createInlineWidgets = function() {
    ...
    elements = document.querySelectorAll('.karen-inline-widget');
    results = [];
    for (i = 0, len = elements.length; i < len; i++) {
      element = elements[i];
      if (!shouldSkipAutoLoadInlineWidget(element)) {
        element.setAttribute('data-processed', true);
        results.push(new Karen.Iframe({
  ...
  createBadgeWidget = function(options) {
    ...
    Karen.destroyBadgeWidget();
    badgeOptions = extractBadgeOptions(options);
    onClickHandler = function() {
      return Karen.showPopupWidget(options.url, 'PopupWidget', options);
    };
    initBadgeOptions = Karen._util.assign({
      onClick: onClickHandler
    }, badgeOptions);
    return Karen.badgeWidget = new Karen.BadgeWidget(initBadgeOptions);
  };
...
(function() {
  Karen.Iframe = (function() {
    ...
    Iframe.prototype.parseOptions = function() {
      ...
      this.options = Karen._util.assign({}, defaultOptions, this.options);
    ...
    Iframe.prototype.getSource = function() {
      var source;
      source = (Karen._url.stripQuery(this.url)) + "?" + (this.getParams());
    ...
    Iframe.prototype.getParams = function() {
      ...
      params = Karen._util.assign(params, this.getUtmParamsFromHost(), this.getParamsFromUrl(), this.getParamsFromOptions());
      parts = [];
      for (key in params) {
        value = params[key];
    ...
    Iframe.prototype.getUtmParamsFromHost = function() {
      ...
      queryStringParams = Karen._url.extractQueryStringParams(window.location.href);
      return Karen._util.pick(queryStringParams, keyFilters);
    };

    Iframe.prototype.getParamsFromUrl = function() {
      return Karen._url.extractQueryStringParams(this.url);
    };

    Iframe.prototype.getParamsFromOptions = function() {
      return Karen._util.assign({}, this.getPrefillParams(), this.getUtmParams());
    };
    ...
    Iframe.prototype.getUtmParams = function() {
      ...
      return Karen._util.snakeCaseKeys(Karen._util.pick(this.options.utm, keyFilters));
    };
    ...
    Iframe.prototype.getPrefillParams = function() {
      ...
      params = Karen._util.snakeCaseKeys(Karen._util.pick(this.options.prefill, keyFilters));
    ...
    Iframe.prototype.formatMobile = function() {
      if (this.inlineStyles) {
        ...
        return this.parent.className += ' karen-mobile';
      }
    };
    ...
    Iframe.prototype.buildSpinner = function() {
      ...
      spinner.className = 'karen-spinner';
...
(function() {
  ...
  Karen.PopupWidget = (function() {
    ...
    PopupWidget.prototype.buildCloseOverlay = function() {
      ...
      node.className = 'karen-close-overlay';
    ...
    PopupWidget.prototype.buildCloseButton = function() {
      var node;
      node = document.createElement('div');
      node.className = 'karen-popup-close';
      node.onclick = this.close;
      return node;
    };
...
(function() {
  Karen.BadgeWidget = (function() {
    ...
    BadgeWidget.prototype.buildContent = function() {
      ...
      element.className = 'karen-badge-content';
      if (this.options.color === '#ffffff') {
        element.className += ' karen-white';
      }
    ...
    BadgeWidget.prototype.buildBranding = function() {
      ...
      element.innerHTML = 'powered by Karen';
      return element;
    };
